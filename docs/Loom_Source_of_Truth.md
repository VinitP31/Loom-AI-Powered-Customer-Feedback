...
- **Calm-but-unresolved convention:** a ticket reporting a real unresolved problem but written in a calm, non-emotional tone should score mildly negative within the Neutral band (approximately -0.2 to -0.3), not 0.0. The label stays `Neutral` — the situation isn't emotionally charged enough to call it `Negative` — but the score should reflect the mild negative lean rather than sitting at dead center, which would falsely read as "no issue at all."
